...
- The user must confirm most phase transitions. Two transitions auto-advance on success and do not prompt:
  - `verify → ai-review` (when all checks pass; failure path still asks the user how to proceed)
  - `ai-review → user-review` (ai-review now only emits findings; the user makes per-finding decisions in `user-review`)
